...
function is_prestress_stable(F::AllTypes; tol_rank_drop::Real=1e-6, tol::Real=1e-10)::Bool
    ...
    point = to_Array(F, F.G.realization)
    flexes = compute_nontrivial_inf_flexes(F.G, point, K_n; tol=tol_rank_drop)
    if size(flexes)[2]==0
        return true
    end
    rigidity_matrix = evaluate.(F.G.jacobian, F.G.variables=>point)
