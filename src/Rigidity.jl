...
function is_inf_rigid(F::AllTypes; tol_rank_drop::Real=1e-8)::Bool
    ...
    trivial_inf_flexes = nullspace(evaluate(typeof(K_n)==ConstraintSystem ? K_n.jacobian : K_n.G.jacobian, (typeof(K_n)==ConstraintSystem ? K_n.variables : K_n.G.variables)=>to_Array(F, F.G.realization)[eachindex( (typeof(K_n)==ConstraintSystem ? K_n.variables : K_n.G.variables))]); atol=tol_rank_drop)
